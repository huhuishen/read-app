...
- 文章类型关键字段（[articles.ts:45-84](file:///d:/dev/reading/read-app/src/lib/models/articles.ts#L45-L84)）：`id, version, isLatest, title, authorId, author, content, coverImage, summary, tags, status("草稿"|"待审核"|"上架"|"下架"), stats{view,mark,comment,vote,rate,rateSum,rateCount,readSeconds}, category{year,month,period,voteEnd}`。
...
- 「首页文章 300 条」实现：生成 300 篇 `status:"上架"` 文章，分布在最近 10 个月（含当月）；对每个月份 `period` 调用 `buildPreview(period, 30, true)`，使 10 个分类各 30 条预览 → 首页合计 300 条。
